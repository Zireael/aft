...
- Contains: Bridge lifecycle management, binary resolution, download, ONNX runtime detection, storage migration, compact formatting, zoom-format rendering
- Key files: `packages/aft-bridge/src/bridge.ts`, `packages/aft-bridge/src/pool.ts`, `packages/aft-bridge/src/resolver.ts`, `packages/aft-bridge/src/downloader.ts`, `packages/aft-bridge/src/onnx-runtime.ts`, `packages/aft-bridge/src/migration.ts`
...
**New FTS5 search lane:** `crates/aft/src/fts5_planner.rs` -- add a new lane to the multi-lane query planner and register it in the `Lane` enum.

**New symbol intelligence module:** `crates/aft/src/symbol_[domain].rs` -- add symbol resolution, insert, or diagnostics logic and export from `lib.rs`.

**New mutation risk factor:** `crates/aft/src/mutation_risk.rs` -- add a new `RiskReason` code and weight in the `classify_mutation_risk` function.
